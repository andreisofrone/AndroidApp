...
{
	public class BarViewController : UIViewController, INChartSeriesDataSource
	{
		NChartView m_view;
		Random m_rand;
		// Brushes.
		...
		public NChartPoint [] Points (NChartSeries series)
		{
			...
			if (drawIn3D) {
				for (int i = 0; i <= 4; ++i)
					for (int j = 0; j <= 4; ++j)
						result.Add (new NChartPoint (NChartPointState.PointStateAlignedToYZWithXYZ ((m_rand.Next () % 30) + 1, i, j), series));
			} else {
				for (int i = 0; i <= 10; ++i)
					result.Add (new NChartPoint (NChartPointState.PointStateAlignedToYWithXY ((m_rand.Next () % 30) + 1, i), series));
			}
